...
// ATENÇÃO: O endpoint agora é /create-payment
app.post("/create-payment", async (req, res) => {
    ...
    if (cleanPhone.length < 12) {
        cleanPhone = "5511987654321"; // Telefone padrão se não for válido
    }
    cleanPhone = cleanPhone.substring(0, 13);

    // --- NOVO: Limpeza e validação do CPF ---
    let cleanDocument = document ? String(document).replace(/\D/g, '') : null; // Garante string e remove não-dígitos
    if (cleanDocument && cleanDocument.length !== 11) {
        console.warn(`CPF ${document} inválido (não tem 11 dígitos). Enviando como null.`);
        cleanDocument = null; // Se não tem 11 dígitos, envia como null
    }
    // --- FIM DA NOVO: Limpeza e validação do CPF ---
    ...
    const payload = {
        ...
        buyer: {
            name: name,
            email: email,
            document: cleanDocument, // <-- Usa o CPF limpo ou null
            phone: cleanPhone
        },
    ...
    try {
        ...
        if (data.data && data.data.pix && data.data.pix.qrcode_base64) {
            ...
            pendingTransactions.set(externalId, {
                ...
                status: 'pending',
                tracking: tracking,
                customer: { name, email, document: cleanDocument, phone: cleanPhone }, // Usa cleanDocument aqui também
                product: product_id && product_name ? { id: product_id, name: product_name } : null,
                offer: offerPayload,
            ...
            const sent = await sendToUTMify(
                ...
                tracking,
                "waiting_payment",
                { name, email, document: cleanDocument, phone: cleanPhone }, // Usa cleanDocument aqui também
